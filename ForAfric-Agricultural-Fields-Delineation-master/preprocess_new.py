...
def prepare_vector(fp, out_crs, clipping_bounds):
        df = (gpd.read_file(str(fp), encoding='cp865')  # danish encoding
             .rename(columns={'Afgroede': 'lc_name', 'Afgkode': 'lc_id', 'Journalnr': 'journalnr'}) #take care here that you are renaming the correct column with correct name
             .drop(['GB'], axis=1)
...
chip_width, chip_height = 1024,1024 #change input chip size here
...
utils.coco.plot_coco(inpath_json=outpath / r'annotations/val2016.json', 
                     inpath_image_folder=outpath / r'images/val2016',end=2)
